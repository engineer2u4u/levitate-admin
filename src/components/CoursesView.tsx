  ...
  return (
    <div style={{ padding: "22px 26px 60px", display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 14, flexWrap: "wrap" }}>
        ...
        <button type="button" className="btn btn-dark" onClick={() => setEditing("new")}>+ New course</button>
      </div>

      {data.courses.length === 0 ? (
        <EmptyState
          title="No courses yet"
          body="A course holds the title, fee and duration. Sessions are the dates you run it on, and enrolments hang off those."
          action={<button type="button" className="btn btn-primary" onClick={() => setEditing("new")}>Create the first course</button>}
        />
      ) : (
        <div className="course-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 12 }}>
          {data.courses.map((c) => {
            ...
            return (
              <div key={c.id} style={{ ...card, padding: "16px 18px", display: "flex", flexDirection: "column", gap: 12 }}>
